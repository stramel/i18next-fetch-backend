...
test.cb('fail to load non-existent language translation', assert => {
  createFetchBackend(
    ...
    err => {
      assert.deepEqual(err, [`failed loading ${server.url}/de/translation.json`]);
      assert.end();
    },
  );
});
...
test.cb('fail to load non-existent namespace translation', assert => {
  createFetchBackend(
    ...
    err => {
      assert.deepEqual(err, [`failed loading ${server.url}/en/mynamespace.json`]);
      assert.end();
    },
  );
});
...
test.cb('fail to load non-existent domain', assert => {
  createFetchBackend(
    ...
    err => {
      assert.deepEqual(err, ['failed making request http://bad-localhost:3000/en/translation.json']);
      assert.end();
    },
  );
});
...
test.cb('calls the callback with an error if the fetch fails', assert => {
  createFetchBackend(
    ...
    err => {
      assert.deepEqual(err, [`failed making request ${server.url}/en/translation.json`]);
      assert.end();
    },
  );
});
